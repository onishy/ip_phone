...
def modify_pitch(m, mcep_file, raw_file):
    """ピッチを変形して再合成
    mが1より大きい => 低い声
    mが1より小さい => 高い声"""
    cmd = "sopr -m %f data | excite -p 80 | mlsadf -m 25 -a 0.42 -p 80 %s | clip -y -32000 32000 | x2x +fs > %s" % (m, mcep_file, raw_file)
    subprocess.call(cmd, shell=True)
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        # どれか一つしか有効にできない
        modify_pitch(1.0, mcep_file, output_file)
